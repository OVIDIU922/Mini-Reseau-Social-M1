...
/*document.addEventListener("DOMContentLoaded", function () {
    ...
    function showConversation(conversation) {
        ...
        conversation.messages.forEach(message => {
            ...
            li.innerHTML = `
                <img src="${message.sender.profilePic}" alt="${message.sender.name}" class="profile-pic">
                <strong>${message.sender.name}</strong> <span>${new Date(message.timestamp).toLocaleString()}</span>
                <p>${message.text}</p>
            `;
            messagesContainer.appendChild(li);
        });
    ...
    newMessageForm.addEventListener("submit", (event) => {
        ...
        li.innerHTML = `
            <img src="${newMessage.sender.profilePic}" alt="${newMessage.sender.name}" class="profile-pic">
            <strong>${newMessage.sender.name}</strong> <span>${new Date(newMessage.timestamp).toLocaleString()}</span>
            <p>${newMessage.text}</p>
        `;
        messagesContainer.appendChild(li);
        messageInput.value = ""; // Réinitialiser le champ de saisie
